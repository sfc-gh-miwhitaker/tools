...
echo "✅ Created .env.server.local (backend) configuration file"
echo ""

# Basic sanity check that we wrote the expected env var
if grep -q '^SNOWFLAKE_PRIVATE_KEY_PEM=' .env.server.local; then
    echo "✅ Private key value written to .env.server.local"
else
    echo "⚠️  WARNING: Expected SNOWFLAKE_PRIVATE_KEY_PEM not found in .env.server.local"
fi
echo ""
...
echo "✅ Created .env.local (frontend) configuration file"
echo ""
...
echo "✅ Generated complete SQL deployment script: $SQL_OUTPUT"
echo ""
echo "=========================================="
echo "🎉 Setup Complete!"
echo "=========================================="
...
echo "=========================================="
echo "✓ .env.server.local created (contains private key)"
...
echo "⚠️  Store rsa_key.pem securely"
echo "⚠️  .gitignore already excludes these files"
echo ""
